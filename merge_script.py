...
# La tua mappatura completa fornita
...
def clean_string(s):
    # Trasforma tutto in maiuscolo e rimuove qualsiasi carattere non alfanumerico
    # Esempio: "Sant'Angelo Le Fratte" diventa "SANTANGELOLEFRATTE"
...
def merge():
    ...
    # Creiamo una versione "pulita" della mappatura comuni per il confronto
    clean_municipality_map = {clean_string(k): v for k, v in MUNICIPALITY_MAP.items()}
    ...
    for feature in geo_data['features']:
        ...
        # Cerchiamo il comune usando la stringa pulita
        zona_id = clean_municipality_map.get(clean_string(nome_geojson))
        ...
        if zona_id:
            ...
            if info:
                ...
                feature['properties']['zona_nome'] = zona_id
                feature['properties']['rischio'] = info.get('rischio_oggi', '')
            else:
                feature['properties']['colore_web'] = "#00ff00"
        else:
            # Se il comune non viene trovato, resta grigio per segnalare l'errore
            feature['properties']['colore_web'] = "#333333" 
